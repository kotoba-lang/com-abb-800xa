(ns abb-800xa.main
  "Kotodama WASM entrypoint for the Abb_800xa clean-room actor (L4) — Clojure port.
  ...
  facts (`abb_800xa.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [kotoba.lang.text :as str]))

(def ns-prefix "abb_800xa")
(def tier "L4")
...
(defn page-limit
  ...
  src/abb_800xa/page_limit.kotoba."
  [requested]
  (if (pos? requested)
...
(def entity-specs
  [{:entity "Tag"        :plural "tags"        :id-prefix "abb800xa_tag"
   ...
   {:entity "ControlLoop" :plural "controlloops" :id-prefix "abb800xa_con"
    :fields [:name :setpoint :processValue :mode]  :required [:name :setpoint]
    :coerce {:setpoint :float :processValue :float} :refs {}}
   {:entity "Batch"      :plural "batches"     :id-prefix "abb800xa_bat"
...
;; --- pagination decision-kernel twins (kotoba: src/abb_800xa/paginate.kotoba) ---
(defn as-int-kernel
  ...
  of abb-800xa.main/as-int stays here as the retained oracle."
  [v]
  (if (pos? v) v 0))
...
(defn emit-facts
  "EAVT facts for one record: {\"abb_800xa.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "abb_800xa-compat" :tier tier :entities entities} 200])
...
(defn start! [] :abb-800xa-compat/ready)
